...
const Korwe = () => {
    const [korwe, setKorwe] = useState([]);
    const dispatch = useDispatch();

    const { username } = useParams();

    const getDataKorwe = async(data) => {
        try {
            const response = await dispatch(getRelawanByJabatan(data)).unwrap().catch((err) => {});
            setKorwe(response.data);
        } catch (error) {
    ...
    useEffect(() => {
        getDataKorwe({ jabatan: 'korwe', page: 1 });
    }, []);
        ...
        <div className="mb-3">
            <div className="card">
                <div className="card-body">
                    <div>
                        <FormTitle title={'DATA KOORDINATOR RW'} />
                    </div>
